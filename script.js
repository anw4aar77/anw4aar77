...
// Function bach t-jbed TOP 50 aghani dyal l-channel b-st3mal order=viewCount
async function fetchChannelVideos(channelId, channelTitle) {
    ...
    try {
        // maxResults=50 & order=viewCount bach y-jbed l-top 50 aghani 3la 7sab l-views
        const response = await fetch(
        ...
        data.items.forEach((item, index) => {
            ...
            songCard.className = "song";
            songCard.innerHTML = `
                <span class="rank-number" style="font-weight: bold; color: #1ed760; width: 30px; text-align: center;">#${index + 1}</span>
                <img src="${song.thumbnail}" style="width: 55px; height: 55px; border-radius: 6px;">
                <div class="song-info" style="flex: 1; margin-left: 10px;">
                    <strong>${song.title}</strong>
                    <span style="display: block; color: #aaa; font-size: 12px;">${song.artist}</span>
                </div>
                <div class="song-actions">
                    <button class="action-btn" onclick="openAddToPlaylistModal('${song.videoId}', '${song.title.replace(/'/g, "\\'")}', '${song.artist.replace(/'/g, "\\'")}', '${song.thumbnail}')"><i class="fa-solid fa-plus"></i></button>
                    <button class="play-btn" onclick='playVideo(${JSON.stringify(song)})'><i class="fa-solid fa-play"></i></button>
...
// Function bach t-l3ab l-channel queue كاملا
function playAllChannelSongs() {
    if (window.currentChannelQueue && window.currentChannelQueue.length > 0) {
        shuffledQueue = [...window.currentChannelQueue];
        currentQueueIndex = 0;
        playVideo(window.currentChannelQueue[0]);
        showToast("Playing channel playlist!");
    }
}
